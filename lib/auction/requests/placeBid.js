...
const { headerLog, hexToArray, stringToHex, jsonToHex, hexToJson, intToHex, uint8ArrayToHex } = require("../../util")
...
const { addAllCellDeps, balanceCapacity,
        ...
        fulfillTransaction, fulfillMalleableTransactionNoSign, hashFromData,
        makeCell, makeConsensusCell, makeDefaultTransaction,
...
// FIXME: There needs to be fee cells as well, which are owned by the auction
// (and hence can be rebased, as we do not need to re-sign the entire tx).
// This will be used the balance the tx.
// For now we use noop cells as a workaround.
async function placeBid(indexer, scriptMetaTable, amount, auctionTxHash) {
    ...
    const bidLockData = createBidLockData(scriptMetaTable, avoum_id, publicKey, bidRequest)
    const bidCell = createBidCell(scriptMetaTable, amount, bidLockData)
    ...
    const { tx_hash: bid_tx_hash } = await fulfillTransaction(bidTransaction);
    ...
    const feeInput = await createNoopCellInput(1000n, indexer, scriptMetaTable)
    transaction = transaction.update("inputs", i => i.push(feeInput))

    let oldBidInput = null
    if (prevTx.outputs.length >= 4) { // Include old bid cell if there was previous bid
        oldBidInput = makeInputCell(auctionTxHash, 2)
    ...
    const escrowedBidCell = makeEscrowed(newBidInput, scriptMetaTable[auctionConfig.AUCTION_ESCROW_LOCK_SCRIPT]["script"])
    transaction = transaction.update("outputs", i => i.push(escrowedBidCell))

    if (oldBidInput) { // Include old bid cell if there was previous bid
        const refundCell = { cell_output: { ...oldBidInput.cell_output, lock: refundLockScript }, data: oldBidInput.data }
        transaction = transaction.update("outputs", i => i.push(refundCell))
    }

    // Include witness
    // const placeHolderWitness = "0x"
	// transaction = transaction.update("witnesses", w => w.push(placeHolderWitness))
	// transaction = transaction.update("witnesses", w => w.push(placeHolderWitness))

    // Include witness for auction request
    const requestWitness = jsonToHex(bidRequest)
	transaction = transaction.update("witnesses", w => w.push(requestWitness)) // indicate this is a new bid

    // Include witness for inputs[2].lock
    const bidLockWitness = jsonToHex(bidLockData)
	transaction = transaction.update("witnesses", w => w.push(bidLockWitness)) // indicate this is a new bid

    console.log("number of witnesses: ", transaction.get("witnesses").toArray().length)

    const { tx_hash } = await fulfillMalleableTransactionNoSign(transaction)
...
function makeEscrowed(prevOutputCell, escrowLockScript) {
    ...
    const escrowedCellOutput = { ...prevOutputCell, cell_output: { ...prevOutputCell.cell_output, lock: escrowLockScript } }
    console.log("updated escrowed cell: ", escrowedCellOutput)
    return escrowedCellOutput
}

function createBidLockData(scriptMetaTable, avoum_id, pub_key, bidRequest) {
    const data = {
        request_hash: hashFromData(bidRequest), // FIXME
        auction_script: scriptAsJson(scriptMetaTable[auctionConfig.AUCTION_AUCTION_TYPE_SCRIPT]["script"]),
        avoum_id,
        pub_key
    }
    console.log("bid lock data: ", data)
    return data
}

function createBidLockScript(scriptMetaTable, bidLockData) {
    const args = uint8ArrayToHex(new Uint8Array(hashFromData(bidLockData).digest))
...
function createBidCell(scriptMetaTable, amount, bidLockData) {
    const data = "0x"
    const lockScript = createBidLockScript(scriptMetaTable, bidLockData)
    const output =
          {
              cell_output:
