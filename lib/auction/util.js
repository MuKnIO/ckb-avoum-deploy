...
const {uint8ArrayToHex, ckbytesToShannons, hexToInt, intToHex, intToU128LeHexBytes,
       headerLog, hexToUint8Array, hexToByte32Array, hashTypeToByteArray, hexToBytesArray, jsonToHex,
       stringToHex, hexToArray, hexToArrayBuffer, u128LeHexBytesToInt, sleep} = require("../util.js");
...
function newAvoumId(outpoint, index) {
    ...
    const tx_hash = hexToArray(outpoint.tx_hash)
...
// NOTE: unverified
function hashWithType(avoumId, typeScript) {
    headerLog("Calling hashWithType")
    console.log("avoumId:", avoumId)
    console.log("typescript:", typeScript)
    let h = sha256.create()

    const codeHash = hexToByte32Array(typeScript.code_hash)
    console.log(codeHash)
    h.update(codeHash)

    const hashType = hashTypeToByteArray(typeScript.hash_type)
    console.log(hashType)
    h.update(hashType)

    const typeScriptArgs = hexToBytesArray(typeScript.args)
    console.log(typeScriptArgs)
    h.update(typeScriptArgs)

    const id = avoumId.unique_hash.digest
    console.log(id)
    h.update(id);

    const digest = h.array()
    console.log("Hashed result: ", digest)
    return digest
}
...
// NOTE: This is the intermediate representation for
// scripts in our cell data (as json objects), not the actual script data structure itself.
// DO NOT use it for that purpose.
function scriptAsJson(script) {
    const code_hash = hexToArray(script.code_hash)
    const args = hexToArray(script.args)
    return {
        args,
...
// NOTE: This is the actual script data structure
...
function makeConsensusData(bid_amount, avoumId, scriptMetaTable) {
    ...
    const sellerLockScript = { code_hash : scriptMetaTable[auctionConfig.AUCTION_NOOP_LOCK_SCRIPT]["codehash"],
                               args: "0x",
                               hash_type: "data" }
    scriptMetaTable[auctionConfig.AUCTION_NOOP_LOCK_SCRIPT]["script"] = sellerLockScript
    const sellerLockScriptJson = scriptAsJson(sellerLockScript)


    let escrowLockScriptHash = scriptMetaTable[auctionConfig.AUCTION_ESCROW_LOCK_SCRIPT]["codehash"]
    const typeHash = hashWithType(avoumId,
                                  scriptMetaTable[auctionConfig.AUCTION_AUCTION_TYPE_SCRIPT].script)
    let escrowLockScript = { code_hash : escrowLockScriptHash, args: uint8ArrayToHex(new Uint8Array(typeHash)), hash_type: "data" }
    scriptMetaTable[auctionConfig.AUCTION_ESCROW_LOCK_SCRIPT]["script"] = escrowLockScript
    const escrowLockScriptJson = scriptAsJson(escrowLockScript)
    ...
    let data = {
        ...
        deadline_block: 0,
        seller_lock_script: sellerLockScriptJson,
        escrow_lock_script: escrowLockScriptJson,
        refund_lock_script: sellerLockScriptJson
    }
...
function makeConsensusCell(amount, avoumId, scriptMetaTable) {
    const outputCapacity = ckbytesToShannons(2000n); // TODO: Somewhat arbitrary, fit to data size.

    const auctionLockScriptHash = scriptMetaTable[auctionConfig.AUCTION_NOOP_LOCK_SCRIPT]["codehash"]
	const lockScript = { args: "0x", code_hash: auctionLockScriptHash , hash_type: "data"}

    const auctionTypeScriptHash = scriptMetaTable[auctionConfig.AUCTION_AUCTION_TYPE_SCRIPT]["codehash"]
	const typeScript = { args: "0x", code_hash: auctionTypeScriptHash , hash_type: "data"}
    scriptMetaTable[auctionConfig.AUCTION_AUCTION_TYPE_SCRIPT].script = typeScript
...
function makeEscrowCell(amount, scriptMetaTable, avoumId) {
    const outputCapacity = ckbytesToShannons(amount);
    ...
    // const typeHash = hashWithType(avoumId,
    //                               scriptMetaTable[auctionConfig.AUCTION_AUCTION_TYPE_SCRIPT].script)
    // const args = jsonToHex(typeHash)

    const escrowLockScript = scriptMetaTable[auctionConfig.AUCTION_ESCROW_LOCK_SCRIPT]["script"]

	// const lockScript = { args, code_hash: escrowLockScriptHash , hash_type: "data"}
    // scriptMetaTable[auctionConfig.AUCTION_ESCROW_LOCK_SCRIPT].script = lockScript

	const output =
    { cell_output:
      { capacity: intToHex(outputCapacity)
      , lock: escrowLockScript
      }
    , data: "0x00"
    };
    return output
...
function makeCell(capacity, lockScript) {
    const outputCapacity = ckbytesToShannons(capacity);
...
async function createNoopCellInput(amount, indexer, scriptMetaTable) {
...
async function fulfillMalleableTransactionNoSign(transactionSkeleton) {
	// Sign the transaction.
    const transaction = createTransactionFromSkeleton(transactionSkeleton)
	...
	const txid = await sendTransaction(auctionConfig.DEFAULT_NODE_URL, transaction, script, indices);
	console.log(`Transaction Sent: ${txid}\n`);
...
function hashFromData(bidRequest) {
    let h = sha256.create()
    let bidRequestData = new Uint8Array(hexToArrayBuffer(jsonToHex(bidRequest)))
    h.update([...bidRequestData])
    const hashedRequest = h.array()
    return {
        digest: hashedRequest
    }
}
...
module.exports = {
    ...
    fulfillTransaction,
    fulfillMalleableTransaction,
    fulfillMalleableTransactionNoSign,
    getCapacity,
    hashFromData,
    hashWithType,
    initializeContext,
