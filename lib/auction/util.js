...
function scriptAsJson(script) {
    ...
    console.log("serialize code hash:", code_hash_ser)
    let args_ser = new Uint8Array(hexToArrayBuffer(script.args))
    args_ser = [...args_ser]
    console.log("serialized args:", args_ser)
    return {
        "args": args_ser,
...
function makeConsensusData(bid_amount, avoumId, scriptMetaTable) {
    ...
    let seller_lock_script = { code_hash : seller_lock_script_hash, args : "0x" }
    let sellerLockScriptJson = scriptAsJson(seller_lock_script)
    ...
    console.log("escrow args: ", args)

	const escrowLockScript = { args, code_hash: escrowLockScriptHash, hash_type: "data"}
...
function makeEscrowCell(amount, scriptMetaTable, avoumId) {
    ...
    const lockScript = scriptMetaTable[auctionConfig.AUCTION_ESCROW_LOCK_SCRIPT].script
