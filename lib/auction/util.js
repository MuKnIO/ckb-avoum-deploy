...
function hashWithType(avoumId, typeScript) {
    ...
    let h = sha256.create()
    h.update(hexToByte32Array(typeScript.code_hash));
    h.update(hashTypeToByteArray(typeScript.hash_type));
    h.update(hexToBytesArray(typeScript.args));
    h.update(avoumId.unique_hash.digest);
...
function makeConsensusData(bid_amount, avoumId, scriptMetaTable) {
    ...
    let escrowLockScript = { code_hash : escrowLockScriptHash, args: jsonToHex(typeHash), hashType: "data" }
...
function makeEscrowCell(amount, scriptMetaTable, avoumId) {
	...
	const lockScript = { args: args, code_hash: escrowLockScriptHash , hash_type: "data"}
