...
function makeConsensusData(bid_amount, avoumId, scriptMetaTable) {
    ...
    let escrowLockScript = { code_hash : escrowLockScriptHash, args: jsonToHex(typeHash), hash_type: "data" }
...
function hashFromData(bidRequest) {
    let h = sha256.create()
    const bidRequestData = new Uint8Array(hexToArrayBuffer(jsonToHex(bidRequest)))
