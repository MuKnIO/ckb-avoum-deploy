...
function mylocate(a) {
	const cd = locateCellDep(a)
	// console.log(cd)
	// // cd['dep_type'] = 'code'
	// console.log(cd)
	// console.log(cd['out_point']['index'])
	// // cd['out_point']['index'] = '0x0'
	// console.log(cd['out_point']['index'])
	return cd
}
...
{
	const tx = transaction.update("cellDeps", (cellDeps)=>cellDeps.push(mylocate({code_hash: DEFAULT_LOCK_HASH, hash_type: "type"})));
	return tx;
...
// TODO: Move to lib
const syncIndexer = async (indexer) => {
	console.log("Indexer is syncing. Please wait.");
    await indexerReady(indexer, (indexerTip, rpcTip)=>console.log(`Syncing ${Math.floor(Number(indexerTip)/Number(rpcTip)*10_000)/100}% completed.`), {timeoutMs: 0, recheckMs: 800});
    console.log("Done")
}
