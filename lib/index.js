// This was taken from Nervos developer-training-course:
// https://github.com/jordanmack/developer-training-course
"use strict";
...
const {ckbytesToShannons, formattedNumber, getRandomInt, headerLog, hexToInt, intToHex, shannonsToCkbytes} = require("./util.js");
...
const addCellDep = (transaction, cellDep) =>
...
function describeTransactionCore(transaction)
{
	const options =
	{
		showCellDeps: true,
...
async function syncIndexer(indexer)
{
	console.log("Indexer is syncing. Please wait.");
	await indexerReady(indexer, (indexerTip, rpcTip)=>console.log(`Syncing ${Math.floor(Number(indexerTip)/Number(rpcTip)*10_000)/100}% completed.`), {timeoutMs: 0, recheckMs: 800});
	console.log("Done")
...
async function sendTransaction(nodeUrl, signedTx, rebaseScript, accountCellIndices)
{
	const rpc = new RPC(nodeUrl);
	...
	try
	{
		...
		// result = await rpc.send_transaction(signedTx, "passthrough");
		if (rebaseScript && accountCellIndices) {
		    result = await rpc.send_transaction(signedTx, "passthrough", rebaseScript, accountCellIndices);
		} else {
			result = await rpc.send_transaction(signedTx, "passthrough");
		}
	}
...
async function waitForConfirmation(nodeUrl, txid, updateProgress=((_status)=>{}), options)
{
...
async function waitForNextBlock(nodeUrl, blocks=1n, updateProgress=((_startTip, _tip)=>{}), options)
{
...
async function waitForTransactionConfirmation(nodeUrl, txid)
{
