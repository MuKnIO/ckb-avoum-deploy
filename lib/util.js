...
{
	const bytes = hexToArrayBuffer(hexString)
...
{
	const jsonStr = hexToString(hexString)
...
function hexToByte32Array(codeHash) {
	// FIXME: Validate the codehash
	return hexToArray(codeHash)
}

// hashType := type | data
function hashTypeToByteArray(hashTypeDescriptor) {
	if (hashTypeDescriptor === "data") {
		return [0]
	} else { // "type"
		return [1]
	}
...
function jsonToHex(jsonObject)
{
	return stringToHex(JSON.stringify(jsonObject))
}
